...
# 训练过的模型路径
resume_path = r"/home/joe/JoeFilesCollections/cap-pytorch/Contrast_model/MobileNetV3LargeOut/1/0.9599999785423279_epoch52normal_acc0.9399999976158142ossc_acc0.9800000190734863.pth"
# 输入图像路径
single_img_path = r'/home/joe/JoeFilesCollections/cap-pytorch/DataSet/MouthCancerDataSet/LabDataSet/makedTifSet/2/test/ossc/285636-HE-B-1-20.tif'
# 绘制的热力图存储路径
save_path = single_img_path.split("/")[-1].split(".")[0] + ".jpg"#r'./testImg.jpg'

# 网络层的层名列表, 需要根据实际使用网络进行修改
layers_names = ["features", "avgpool", "classifier"]#['conv1', 'bn1', 'relu', 'maxpool', 'layer1', 'layer2', 'layer3', 'layer4', 'avgpool']
# 指定层名
out_layer_name = "features"
...
def draw_CAM(model, img_path, save_path, transform=None, visual_heatmap=False, out_layer=None):
    ...
    #print(model.backbone._modules.items())
...
if __name__ == '__main__':
    ...
    net = myMobilieNet(2).to(torch.device("cuda"))
    checkpoint = torch.load(resume_path)
    net.load_state_dict(checkpoint)
    draw_CAM(net, single_img_path, save_path, transform=train_transform, visual_heatmap=True, out_layer=out_layer_name)
    # img = Image.open(single_img_path).convert('RGB')
    # if transform:
    #     img = transform(img).cuda()
    # img = img.unsqueeze(0)
    # print(F.softmax(net(img)[0]))